...
window.addEventListener("load", function() {
	const createElement = document.createElementNS.bind(document, document.getElementsByTagName("html")[0].namespaceURI),
	...
	addCategory.addEventListener("click", function() {
		...
		for (let i = 0; i < numRows; i++) {
			const input = values.appendChild(createElement("li")).appendChild(createElement("input"));
	...
	addCategory.click();
		...
		const categories = Array.from(info.getElementsByTagName("div")).map(function(cat) {
		...
		categories.slice(1).forEach(function(cat) {
			const catTitle = firstRow.appendChild(createElement("th"));
			catTitle.textContent = cat.Title;
			catTitle.setAttribute("colspan", numRows);
			cat.Values.forEach(function(title, pos) {
				const cell = secondRow.appendChild(createElement("th"));
		...
		categories.slice(2).concat(categories[0]).reverse().forEach(function(cat, pos) {
			...
			cat.Values.forEach(function(title, cpos) {
				...
				const cell = row.appendChild(createElement("th"));
				cell.textContent = title;
				if (cpos === 0) {
					cell.setAttribute("class", "first");
				}
				categories.slice(1, categories.length - pos).forEach(function(mcat) {
					const columnCatTitle = mcat.Title;
					mcat.Values.forEach(function(val, i) {
						const elm = row.appendChild(createElement("td")),
						      cell = new Cell(elm),
						      columnTitle = val;
						cells.push(cell);
				...
				if (pos > 0) {
					const span = row.appendChild(createElement("td"));
		...
		addRule.addEventListener("click", (function() {
			const ruleFunc = function(catA, catB, rowB, catC, rowC, not, leftRight, far) {
				...
				return changed;
			      ...
			      ruleParts = ["In Category ", "???", ", Category ", "???", ", Row ", "???", " is ", "???", " Category ", "???", ", Row ", "???", "."].map(t => {const s = rule.appendChild(createElement("span"));s.textContent = t; return s}).filter((a, i) => i&1 == 1),
				...
				const l = content.appendChild(createElement("label")),
			...
			categories.forEach(cat => {
				const o = catFrag.appendChild(createElement("option")),
				...
				cat.Values.forEach(v => {
					const o = g.appendChild(createElement("option"));
					o.textContent = v;
			...
			catASel.addEventListener("change", function() {
				...
				const val = catASel.selectedIndex;
			...
			["", "Within", "Exactly"].forEach(t => {
				const o = inputs[4].appendChild(createElement("option"));
			...
			for (let i = 1; i < numRows; i++) {
				const o = inputs[5].appendChild(createElement("option"));
				o.setAttribute("value", i);
				o.textContent = i;
			}
			inputs[5].setAttribute("disabled", "disabled");
			["", "After", "Before"].forEach(t => {
				const o = inputs[6].appendChild(createElement("option"));
				o.setAttribute("value", t);
				o.textContent = t;
			});
			...
			inputs[6].addEventListener("change", writeRule);
				...
				const t = rulesList.appendChild(createElement("tr")),
